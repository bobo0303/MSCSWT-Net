...
class NonSaturatingWithR1():
    def __init__(self, gp_coef=0.001,  mask_as_fake_target=True, allow_scale_mask=True,
    ...
    def generator_loss(self, discr_fake_pred: torch.Tensor, mask=None) \
            -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:    #LG
    ...
    def discriminator_loss(self, real_batch: torch.Tensor, fake_batch: torch.Tensor,
        ...
        real_loss = F.softplus(-discr_real_pred)    #softplus 激活函數
        ...
        sum_discr_loss = real_loss + fake_loss   #LD+LGP+LG
    ...
    def discriminator_real_loss(self, real_batch: torch.Tensor, discr_real_pred: torch.Tensor): #(LD的第一項) +GP

        real_loss = F.softplus(-discr_real_pred)    #https://pytorch.org/docs/stable/generated/torch.nn.Softplus.html
        grad_penalty = make_r1_gp(discr_real_pred, real_batch) * self.gp_coef
        sum_discr_loss = real_loss + grad_penalty
        # sum_discr_loss = real_loss
